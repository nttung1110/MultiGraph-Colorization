...
def merge_graphs(graph1, graph2, new_edges, true_edges):
    ...
    new_edges = torch.tensor(edge_list, device=graph1.edge_index.device, dtype=torch.long)
    true_edges = torch.tensor(true_edge_list, device=graph1.edge_index.device, dtype=torch.long)
