...
from data.data_loader_multigraph_geek import GMDataset, get_dataloader

from utils.evaluation_metric import matching_accuracy_from_lists, f1_score, get_pos_neg_from_lists

from eval_infer_geek import eval_model
...
def train_eval_model(model, criterion, optimizer, dataloader, num_epochs, resume=False, start_epoch=0):
    ...
    # for k, inputs in enumerate(dataloader["train"]):
    #     print(inputs)
    ...
    if cfg.evaluate_only:
       # assert resume
        ...
        accs, f1_scores = eval_model(model, dataloader["test"], 6)
        acc_dict = {
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for inputs in dataloader["train"]:
            ...
            tyler_imgs = inputs["tyler_imgs"] # processed image which is appropriate to tyler model
...
if __name__ == "__main__":
    ...
    # randomly select cuda, can be re-specified in command line
