...
class Generator():
    ...
    def process(self, colored_a, sketch_a, crop_bbox=True):
        """
        ...
        self.augmentor.gen_augment_param(key_name='a2b', key_params={
            'affine': {'ang_range':self.max_ang_range, 'shear_range':self.max_shear_range,
                       'trans_range':trans_range, 'org_shape':(self.resized_h, self.resized_w)},
            'tps': {'dshape':(self.resized_h, self.resized_w)}
        })

        # . b from a
        mask_a2b = self.augmentor.augment(key_name='a2b', image=mask_a)
        colored_b = self.augmentor.augment(key_name='a2b', image=colored_a)
        sketch_b = self.augmentor.augment(key_name='a2b', image=sketch_a)

        mask_b, components_b = self.component_wrapper.extract_on_color_image(colored_b)
        get_component_color(components_b, colored_b, mode=ComponentWrapper.EXTRACT_COLOR)

        positive_pairs = self._get_positive_a2b(components_a, mask_a, components_b, mask_b, mask_a2b)

        return (colored_a, mask_a, components_a, sketch_a), (colored_b, mask_b, components_b, sketch_b), positive_pairs

if __name__ == '__main__':
    image_path = "/mnt/ai_filestore/home/zed/multi-graph-matching/Geek_data/processed_HOR02/hor02_127/color/A0008.png"
    sketch_path = "/mnt/ai_filestore/home/zed/multi-graph-matching/Geek_data/processed_HOR02/hor02_127/sketch/A0008.png"
    ...
    positive_pairs = \
        generator.process(colored_a, sketch_a, crop_bbox=True)
