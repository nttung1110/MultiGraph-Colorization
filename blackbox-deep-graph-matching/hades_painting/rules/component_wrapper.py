...
def get_moment_features_v1(components, mask):
...
class ComponentWrapper:
    ...
    def extract_on_color_image(self, input_image):
        ...
        for unique in uniques:
            ...
            num_vis_components = 0
            limit_vis_components = 1
            ...
            for region in regions:
                ...
                # print((region.image.astype(np.uint8)*255).shape)
                # if num_vis_components < limit_vis_components:
                mask[region.coords[:, 0], region.coords[:, 1]] = index + 1
                #     num_vis_components += 1
                index += 1
                # print(index)
            vis_idx += 1

        # print("Num components:", index)
    ...
    def extract_on_sketch_v3(self, sketch):
        ...
        for region in regions[1:]:
            ...
            # print((region.image.astype(np.uint8)*255).shape)
            # components[index]["image"] = components[index]["image"][:,:,0]
            mask[region.coords[:, 0], region.coords[:, 1]] = index + 1
            index += 1
